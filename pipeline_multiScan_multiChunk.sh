...
run_process_vdif() {
    ...
    for i in ${ifs};do
        /home/franz/git/frb-baseband/process_vdif.py ${source} ${workdir}/${experiment}_${st}_no0${scan}_IF${i}_skip${skip}s.vdif  \
...
for scan in "${scans[@]}";do
    ...
    for i in `seq 1 2 ${nif}`;do
        # odd IFs first
        vdifnme=${workdir_odd}/${experiment}_${st}_no0${scan}_IF${i}_skip${skip}s.vdif
        vdif_files=${vdif_files}${vdifnme}" "
        if [ ! -f ${vdifnme} ];then
            msg "Splitting the VDIF."
            ${spif2file} ${experiment} ${st} ${scan} ${nif} ${mode} ${skip} ${length} ${flipIF}
    	if [[ $? -eq 1 ]];then
    	    exit 1
    	fi
        ...
        # then even IFs
        let n=$i+1
        vdifnme=${workdir_even}/${experiment}_${st}_no0${scan}_IF${n}_skip${skip}s.vdif
        vdif_files=${vdif_files}${vdifnme}" "
        if [ ! -f ${vdifnme} ];then
    ...
    filfile=${experiment}_${st}_no0${scan}_IFall_skip${skip}s_vdif_pol${pol}.fil
	...
	splice ${splice_list} > ${outdir}/${filfile} && \
            rm -rf ${workdir_even}/${experiment}_${st}_no0${scan}_IF*_skip${skip}s.vdif \
               ${workdir_odd}/${experiment}_${st}_no0${scan}_IF*_skip${skip}s.vdif && \
